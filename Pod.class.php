...
USAGE:
$Record = new Pod($post_ID);
echo $Record->get_field('summary');
==================================================
*/
class Pod
{
...
?>
    <div style="float:left; width:30%; font-weight:bold"><?php echo $name; ?></div>
    ...
    function showTemplate($tpl)
    {
        if ('list' == $tpl || 'detail' == $tpl)
        {
            ...
            if ('list' == $tpl)
            {
                ...
                {
                    echo preg_replace_callback("/({@(.*?)})/m", array($this, "magic_swap"), $row['template']);
                }
            }
            elseif ('detail' == $tpl)
            {
