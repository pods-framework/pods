...
{
    $result = mysql_query("SELECT id FROM wp_pod_types WHERE name = '$pickval' LIMIT 1");
    if (0 < mysql_num_rows($result))
    {
        ...
        if (0 < mysql_num_rows($result))
        {
            ...
            $result = mysql_query("SELECT id, name FROM wp_pod_fields WHERE datatype = $sister_datatype AND pickval = '$datatype_name'");
            if (0 < mysql_num_rows($result))
            {
