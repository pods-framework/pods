...
class Pods implements Iterator {
	...
	public function field( $name, $single = null, $raw = false ) {
		...
		if ( in_array( $params->name, array( '_link', 'detail_url' ) ) || ( in_array( $params->name, array(
					'permalink',
					'the_permalink'
				) ) && in_array( $this->pod_data['type'], array(
					'post_type',
					'taxonomy',
					'media',
					'user',
										...
										if ( is_array( $item ) && isset( $item[ $field ] ) ) {
											...
											if ( $table['field_id'] == $field ) {
												...
												$value[] = $item->{$field};
														...
														'attachment',
														'media'
													) ) || in_array( $last_type, PodsForm::file_field_types() ) ) ) || ( in_array( $field, array(
													'_link',
													'detail_url'
