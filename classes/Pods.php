...
class Pods implements Iterator {
	...
	public function field( $name, $single = null, $raw = false ) {
		...
		if ( in_array( $params->name, array( '_link', 'detail_url' ) ) || ( in_array(
			$params->name, array(
				'permalink',
				'the_permalink',
			)
			...
			if ( false === $object_field_found ) {
				...
				if ( isset( $this->fields[ $params->name ] ) ) {
					...
					if ( in_array( $this->fields[ $params->name ]['type'], $tableless_field_types ) ) {
						...
						$this->fields[ $params->name ]['type'], array(
							'boolean',
							'number',
							'currency',
						)
				...
				if ( ! isset( $this->fields[ $params->name ] ) || ! in_array( $this->fields[ $params->name ]['type'], $tableless_field_types ) || $simple ) {
					...
					foreach ( $params->traverse as $key => $field ) {
						...
						// Tableless handler
						if ( $field_exists && ( ! in_array(
							$all_fields[ $pod ][ $field ]['type'], array(
								'pick',
								'taxonomy',
								'comment',
							)
							...
							if ( ! empty( $pick_object ) && ( ! empty( $pick_val ) || in_array( $pick_object, $related_pick_objects ) ) ) {
								if ( 'pod' === $pick_object ) {
									$pod = $pick_val;
								} else {
...
// arrays
							...
							if ( in_array( $last_type, $tableless_field_types ) || in_array(
								$last_type, array(
									'boolean',
									'number',
									'currency',
							...
							if ( empty( $data ) ) {
								...
								if ( $is_field_output_full ) {
									...
									foreach ( $data as $item_id => $item ) {
										...
										} elseif ( ( ( false !== strpos( $full_field, '_src' ) || 'guid' === $field ) && ( in_array(
											$table['type'], array(
												'attachment',
												'media',
											)
										) || in_array( $last_type, PodsForm::file_field_types() ) ) ) || ( in_array(
											$field, array(
												'_link',
												'detail_url',
											)
										...
										} elseif ( false !== strpos( $full_field, '_img' ) && ( in_array(
											$table['type'], array(
												'attachment',
												'media',
											)
	...
	public function remove_from( $field, $value = null, $id = null ) {
		...
		if ( in_array( $this->fields[ $field ]['type'], PodsForm::tableless_field_types() ) ) {
			...
			// Number fields
			// Number fields don't support empty for removing
			if ( empty( $value ) ) {
				return $id;
			}
