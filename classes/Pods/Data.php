...
class Pods_Data {
	...
	public static function init( $pod = null, $id = 0, $strict = true ) {
		...
		if ( ( true !== $pod && null !== $pod ) || 0 != $id ) {
			// Get Pod-based Pods_Data (don't store it)
			return new Pods_Data( $pod, $id, $strict );
		} elseif ( ! is_object( self::$instance ) ) {
			// Get generic Pods_Data that's a singleton
			self::$instance = new Pods_Data();
		} else {
			// Reset generic singleton vars in case they have changed
	...
	public function __construct( $pod = null, $id = 0, $strict = true ) {
		...
		if ( ! empty( $pod ) ) {
			...
			$api->display_errors =& self::$display_errors;

			// Get Pod data
			$pod = pods_clean_name( $pod, true, false );

			$pod_data = $api->load_pod( array( 'name' => $pod ), false );

			if ( empty( $pod_data ) ) {
				if ( true === $strict ) {
					pods_error( 'Pod not found', $this );
				}
			...
			$this->pod      = $pod_data['name'];
			$this->pod_id   = $pod_data['id'];
			$this->fields   = $pod_data['fields'];

			if ( ! empty( $this->pod_data['detail_url'] ) ) {
			...
			foreach ( $table_data as $key ) {
				if ( ! empty( $this->pod_data[ $key ] ) ) {
					$this->{$key} = $this->pod_data[ $key ];
				}
			}
	...
	public function table( $table, $object = '' ) {
		...
		if ( ! empty( $object_type ) ) {
			$table = $api->get_table_info( $object_type, $object );
		}

		if ( ! empty( $table ) && is_array( $table ) ) {
			$table['id']            = pods_v_sanitized( 'id', $table['pod'], 0, true );
			$table['name']          = pods_v_sanitized( 'name', $table['pod'], $table['object_type'], true );
			$table['type']          = pods_v( 'type', $table['pod'], $table['object_type'], true );
			$table['storage']       = pods_v( 'storage', $table['pod'], ( 'taxonomy' == $table['object_type'] ? 'none' : 'meta' ), true );
			$table['fields']        = pods_v( 'fields', $table['pod'], array() );
			$table['object_fields'] = pods_v( 'object_fields', $table['pod'], $api->get_wp_object_fields( $table['object_type'] ), true );
	...
	public function insert( $table, $data, $format = null ) {
		...
		// @todo Needs hook doc
		list( $table, $data, $format ) = apply_filters( 'pods_data_insert', array( $table, $data, $format ), $this );
	...
	public function update( $table, $data, $where, $format = null, $where_format = null ) {
		...
		// @todo Needs hook doc
		list( $table, $data, $where, $format, $where_format ) = apply_filters( 'pods_data_update',
			array(
				$table,
				$data,
				$where,
	...
	public function delete( $table, $where, $where_format = null ) {
		...
		// @todo Needs hook doc
		list( $sql, $where ) = apply_filters( 'pods_data_delete',
			array(
				$sql,
				array_values( $where )
			),
	...
	public function calculate_totals() {
		...
		// @todo Needs hook doc
		$total = apply_filters( 'pods_data_select_total', $total, $this );
	...
	public function build( $params ) {
		...
		} elseif ( false === $params->strict && ! empty( $this->join ) ) {
			$params->join = (array) $this->join;
		} else {
			$params->join = array();
		}
		...
		if ( ! empty( $params->search ) && ! empty( $params->fields ) ) {
						...
						if ( 'pick' == $attributes['type'] && ! in_array( pods_v( 'pick_object', $attributes ), $simple_tableless_objects ) ) {
							if ( false === $params->search_across_picks ) {
								...
								if ( empty( $attributes[ 'table_info' ] ) ) {
									$attributes['table_info'] = $api->get_table_info( pods_v( 'pick_object', $attributes ), pods_v( 'pick_val', $attributes ) );
								}
			...
			foreach ( $params->filters as $filter ) {
				...
				$field      = pods_v_sanitized( 'name', $attributes, $filter, true );
	...
	public function reorder( $table, $weight_field, $id_field, $ids ) {
		...
		list( $table, $weight_field, $id_field, $ids ) = $this->do_hook( 'reorder',
			array(
				$table,
				$weight_field,
				$id_field,
	...
	/**
	 * ToDo: See if we can determine the reason for and impact of changing the default of explicit_set from true to null
	 *
	 * Fetch a new row for the current pod_data
	 *
	 * @param int  $row          Row number to fetch
	 * @param bool|null $explicit_set Whether to set explicitly (use false when in loop)
	 *
	 * @return mixed
	 *
	 * @since 2.0
			...
			if ( 0 === strpos( $this->pod, '_pods_' ) ) {
				...
				if ( false !== $row && is_array( $row ) ) {
					...
					if ( ! did_action( 'init' ) ) {
						...
						// @todo Copy hook doc from WP core
	...
	public static function query( $sql, $error = 'Database Error', $results_error = null, $no_results_error = null ) {
		...
		if ( is_array( $sql ) ) {
			...
			if ( 1 == pods_v( 'pods_debug_sql_all', 'get', 0 ) && pods_is_admin( array( 'pods' ) ) ) {
	...
	public static function get_tables( $wp_core = true, $pods_tables = true ) {
		...
		foreach ( $list_tables as $table ) {
			...
			if ( ! $pods_tables && 0 === ( strpos( $table, $wpdb->prefix . rtrim( self::$prefix, '_' ) ) ) ) {
				continue;
			} elseif ( ! $wp_core && in_array( $table, $core_wp_tables ) ) {
				continue;
			} else {
				$tables[] = $table;
			}
	...
	public static function get_column_data( $column_name, $table ) {
		...
		$column_data = array_map( 'get_object_vars', $column_data );
	...
	public static function prepare( $sql, $data ) {
		...
		// @todo Needs hook doc, do we need this hook though?
	...
	public static function query_field( $field, $q, $pod = null, &$params = null ) {
		...
		$field_name            = trim( pods_var_raw( 'field', $q, pods_v( 'key', $q, $field, true ), null, true ) );
	...
	function traverse_recurse( $traverse_recurse ) {
		...
		if ( ! isset( $pod_data['fields'][$field] ) ) {
			...
			if ( 'post_type' == $pod_data['type'] && isset( $pod_data['object_fields'][$field] ) && in_array( $pod_data['object_fields'][$field]['type'], $tableless_field_types ) ) {
				$traverse = $pod_data[ 'object_fields' ][ $field ]; // @todo Indirect modification
			} elseif ( 'meta_value' === $last && in_array( $pod_data[ 'type' ], array( 'post_type', 'media', 'user', 'comment' ) ) ) {
				$traverse = Pods_Form::field_setup( array( 'name' => $field ) ); // @todo Indirect modification
			} else {
