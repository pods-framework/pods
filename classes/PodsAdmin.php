...
class PodsAdmin {
	...
	public function upgrade_notice() {
		...
		echo sprintf( __( '<strong>NOTICE:</strong> Pods %s requires your action to complete the upgrade. Please run the <a href="%s">Upgrade Wizard</a>.', 'pods' ), esc_html( PODS_VERSION ), esc_url( admin_url( 'admin.php?page=pods-upgrade' ) ) );
		echo '</p></div>';
	}
		...
		if ( 'post_type' == $pod['type'] ) {
			...
			$options['admin-ui'] = array(
				...
				'ui_actions_enabled'   => array(
					...
					'help'             => __( 'help', 'pods' ),
					'type'             => 'pick',
					'default'          => ( 1 == pods_var( 'ui_export', $pod ) ? array(
						'add',
						'edit',
						'duplicate',
						'delete',
						'export'
					) : array( 'add', 'edit', 'duplicate', 'delete' ) ),
					'data'             => array(
						'add'       => __( 'Add New', 'pods' ),
						'edit'      => __( 'Edit', 'pods' ),
	...
	public function admin_capabilities( $capabilities ) {
		...
		$pods       = pods_api()->load_pods( array(
			'type'       => array( 'settings', 'post_type', 'taxonomy' ),
			'fields'     => false,
			'table_info' => false
		) );
	 ...
	 *
	 * @param null|string|array $pod             . Optional. Which Pod(s) to get configuration for. Can be a the name
	 *                                           of one Pod, or an array of names of Pods, or null, which is the
	 *                                           default, to profile all Pods.
	 * @param bool              $full_field_info Optional. If true all info about each field is returned. If false,
	...
	protected function rest_admin() {
		...
		if ( function_exists( 'register_rest_field' ) ) {
			add_filter( 'pods_admin_setup_edit_field_options', array(
				$this,
