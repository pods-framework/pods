    ...
    class PodMeta
    {
        ...
        function __construct ()
        {
            ...
            if (defined('PODS_DEVELOPER')) {
                ...
                elseif ( 'profile.php' == $page || 'users.php' == $page ) {
                    $this->user_setup();
                }
            }
        }
        ...
        function profile_fields ()
        {
            ...
            for ( $i = 0; $i < count( $data[ 'fields' ] ); $i++ ) {
                ob_start();
                $this->show_input( $data[ 'fields' ][ $i ], get_user_meta( $_GET[ 'user_id' ], $data[ 'fields' ][ $i ]->name, true ) );
        ...
        function add_meta_boxes ()
        {
            ...
            if ( !empty( $pts ) ) {
                ...
                if ( !empty( $pod_fields ) ) {
                    ...
                    add_meta_box( $post->post_type . '-pods-fields', __( $post_type->labels->singular_name, 'pods' ), array( $this,
                                                                                                                           'mb_callback' ), $post->post_type, 'normal', 'high', $pod_fields );
                }
            }
        }
        ...
        function taxonomy_meta_setup ()
        {
            foreach ( $this->taxonomies as $taxonomy ) {
                add_action( $taxonomy[ 'name' ] . '_edit_form_fields', array( $this,
                                                                            'tax_fields' ) );
                add_action( $taxonomy[ 'name' ] . '_add_form_fields', array( $this,
                                                                           'tax_fields' ) );
                add_action( 'edited_' . $taxonomy[ 'name' ], array( $this,
                                                                  'tax_save' ) );
                add_action( 'created_' . $taxonomy[ 'name' ], array( $this,
                                                                   'tax_save' ) );
            }
        }
        ...
        function tax_save ()
        {
            ...
            pods_debug($_POST, false, 'taxonomy save');
        }
        ...
        function save_post ( $post_id )
        {
            ...
            if ( !empty( $pts ) ) {
                foreach ( $pts as $field ) {
                    ...
                    elseif ( $field->type == 'pick' && strstr( $field->pick_object, 'taxonomy' ) ) {
