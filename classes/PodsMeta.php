...
class PodsMeta {
	...
	public function cache_pods() {
		...
		self::$advanced_content_types = pods_api()->load_pods(
			array(
				'type'    => 'pod',
				'refresh' => true,
			)
		);
		self::$post_types             = pods_api()->load_pods(
			array(
				'type'    => 'post_type',
				'refresh' => true,
			)
		);
		self::$taxonomies             = pods_api()->load_pods(
			array(
				'type'    => 'taxonomy',
				'refresh' => true,
			)
		);
		self::$media                  = pods_api()->load_pods(
			array(
				'type'    => 'media',
				'refresh' => true,
			)
		);
		self::$user                   = pods_api()->load_pods(
			array(
				'type'    => 'user',
				'refresh' => true,
			)
		);
		self::$comment                = pods_api()->load_pods(
			array(
				'type'    => 'comment',
				'refresh' => true,
			)
		);
		self::$settings               = pods_api()->load_pods(
			array(
				'type'    => 'settings',
				'refresh' => true,
			)
	...
	public function meta_post_add( $post_type, $post = null ) {
		...
		foreach ( $groups as $group ) {
			...
			if ( $field_found ) {
				...
				add_meta_box(
					'pods-meta-' . sanitize_title( $group['label'] ), $group['label'], array(
						$this,
						'meta_post',
	...
	public function save_post( $post_id, $post, $update = null ) {
		...
		if ( is_bool( $update ) ) {
			$is_new_item = ! $update;
		} elseif ( isset( self::$old_post_status[ $post->post_type ] ) && in_array(
			self::$old_post_status[ $post->post_type ], array(
				'new',
				'auto-draft',
			), true
		) ) {
	...
	public function add_meta( $object_type, $_null = null, $object_id = 0, $meta_key = '', $meta_value = '', $unique = false ) {
		...
		if ( in_array( $object['fields'][ $meta_key ]['type'], PodsForm::tableless_field_types() ) ) {
			...
			$pod->save(
				$meta_key, $meta_value, $object_id, array(
	...
	public function delete_meta( $object_type, $_null = null, $object_id = 0, $meta_key = '', $meta_value = '', $delete_all = false ) {
		...
		if ( ! empty( $meta_value ) && in_array( $object['fields'][ $meta_key ]['type'], PodsForm::tableless_field_types() ) ) {
			...
			$pod->save(
				array( $meta_key => null ), null, $object_id, array(
					'podsmeta_direct' => true,
					'error_mode'      => 'false',
				)
			);
