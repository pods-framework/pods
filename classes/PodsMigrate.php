    ...
    */
    ...
    public function import_package ( $data = null, $parse = true ) {
        if ( !empty( $data ) )
            $this->data = $data;
        if ( false !== $parse )
            $this->parse_package( $this->data );
        if ( empty( $this->parsed ) )
            return false;
        $api = pods_api();
        $data = array();
        if ( isset( $this->parsed[ 'pods' ] ) )
            $data[ 'pods' ] = array();
        foreach ( $this->parsed[ 'pods' ] as $pod ) {
            $data[ 'pods' ][] = $api->save_pod( $pod );
        }
        if ( isset( $this->parsed[ 'templates' ] ) ) {
            $data[ 'templates' ] = array();
            foreach ( $this->parsed[ 'templates' ] as $template ) {
                $data[ 'templates' ][] = $api->save_template( $template );
            }
        }
        if ( isset( $this->parsed[ 'pages' ] ) ) {
            $data[ 'pages' ] = array();
            foreach ( $this->parsed[ 'pages' ] as $page ) {
                $data[ 'pages' ][] = $api->save_page( $page );
            }
        }
        if ( isset( $this->parsed[ 'helpers' ] ) ) {
            $data[ 'helpers' ] = array();
            foreach ( $this->parsed[ 'helpers' ] as $helper ) {
                $data[ 'helpers' ][] = $api->save_helper( $helper );
            }
        }
        if ( empty( $data ) )
            return false;
    ...
    public function parse_package ( $data = null ) {
        ...
        if ( !is_array( $this->data ) )
            $this->data = @json_decode( stripslashes( $this->data ), true );
        ...
        if ( isset( $this->data[ 'meta' ] ) && is_array( $this->data[ 'meta' ] ) )
            $data[ 'meta' ] = array_merge( $data[ 'meta' ], $this->data[ 'meta' ] );
        if ( isset( $this->data[ 'pods' ] ) ) {
            $data[ 'pods' ] = array();
            foreach ( $this->data[ 'pods' ] as $pod ) {
                foreach ( $pod as $params ) {
                    if ( isset( $params[ 'before_helpers' ] ) )
                        $params[ 'pre_save_helpers' ] = $params[ 'before_helpers' ];
                    if ( isset( $params[ 'after_helpers' ] ) )
                        $params[ 'post_save_helpers' ] = $params[ 'after_helpers' ];
                    $data[ 'pods' ][] = $params;
                }
            }
        }
        if ( isset( $this->data[ 'templates' ] ) ) {
            ...
            foreach ( $this->data[ 'templates' ] as $template ) {
                foreach ( $template as $name => $code ) {
                    $params = array( 'name' => $name, 'code' => $code );
                    $data[ 'templates' ][] = $params;
                }
            }
        }
        if ( isset( $this->data[ 'pod_pages' ] ) || isset( $this->data[ 'pages' ] ) ) {
            if ( isset( $this->data[ 'pod_pages' ] ) )
                $this->data[ 'pages' ] = $this->data[ 'pod_pages' ];
            $data[ 'pages' ] = array();
            ...
            foreach ( $this->data[ 'pages' ] as $page ) {
                foreach ( $page as $attributes ) {
                    $params = array_merge( array( 'uri' => '', 'page_title' => '', 'page_template' => '', 'phpcode' => '', 'precode' => '' ), $attributes );
                    if ( isset( $params[ 'title' ] ) )
                        $params[ 'page_title' ] = $params[ 'title' ];
                    if ( isset( $params[ 'template' ] ) )
                        $params[ 'page_template' ] = $params[ 'template' ];
                    if ( !empty( $params[ 'page_template' ] ) && !isset( $page_templates[ $params[ 'page_template' ] ] ) )
                        $params[ 'page_template' ] = '';
                    $data[ 'pages' ][] = $params;
                }
            }
        }
        if ( isset( $this->data[ 'helpers' ] ) ) {
            $data[ 'helpers' ] = array();
    ...
    */
    ...
    public function build_package ( $data = null ) {
        ...
        if ( isset( $this->data[ 'pods' ] ) || !empty( $this->data[ 'pods' ] ) ) {
            ...
            $data[ 'pods' ] = array();
            foreach ( $this->data[ 'pods' ] as $id ) {
                $item = $api->load_pod( array( 'id' => $id ) );
                $data[ 'pods' ][ $item[ 'name' ] ] = $item;
            }
        }
        if ( isset( $this->data[ 'templates' ] ) || !empty( $this->data[ 'templates' ] ) ) {
            $this->data[ 'templates' ] = explode( ',', $this->data[ 'templates' ] );
            $data[ 'templates' ] = array();
            foreach ( $this->data[ 'templates' ] as $id ) {
                $item = $api->load_template( array( 'id' => $id ) );
                $data[ 'templates' ][ $item[ 'name' ] ] = $item;
            }
        }
        if ( isset( $this->data[ 'pages' ] ) || !empty( $this->data[ 'pages' ] ) ) {
            $this->data[ 'pages' ] = explode( ',', $this->data[ 'pages' ] );
            $data[ 'pages' ] = array();
            foreach ( $this->data[ 'pages' ] as $id ) {
                $item = $api->load_page( array( 'id' => $id ) );
                $data[ 'pages' ][ $item[ 'name' ] ] = $item;
            }
        }
        if ( isset( $this->data[ 'helpers' ] ) || !empty( $this->data[ 'helpers' ] ) ) {
            $this->data[ 'helpers' ] = explode( ',', $this->data[ 'helpers' ] );
            $data[ 'helpers' ] = array();
            foreach ( $this->data[ 'helpers' ] as $id ) {
                $item = $api->load_helper( array( 'id' => $id ) );
                $data[ 'helpers' ][ $item[ 'name' ] ] = $item;
            }
        }
        if ( empty( $data ) )
            return false;
