...
class Pods_Templates_Frontier {
	...
	function add_metaboxes( $slug, $post = false ){
		...
		if(!empty($post)){
			if(!in_array($post->post_type, array('_pods_template'))){return;}
		}else{
			$screen = get_current_screen();
			if(!in_array($screen->base, array('_pods_template'))){return;}
		}
		...
		// add metabox		
		add_meta_box('view_template', __( 'Template', 'pods' ), array($this, 'render_metaboxes_custom'), '_pods_template', 'normal', 'high', array( 'slug' => 'view_template', 'groups' => array() ) );
		add_meta_box('pod_reference', __( 'Pod Reference', 'pods' ), array($this, 'render_metaboxes_custom'), '_pods_template', 'side', 'default', array( 'slug' => 'pod_reference', 'groups' => array() ) );
	...
	function save_post_metaboxes($pid, $post){
		...
		if(!isset($_POST['pods_templates_metabox']) || !isset($_POST['pods_templates_metabox_prefix'])){return;}
