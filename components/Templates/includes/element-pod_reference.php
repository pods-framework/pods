<div class="pod-queries-tools"><?php
	...
	$api   = pods_api();
	$_pods = $api->load_pods();
	echo '<select name="pod_reference[pod]" id="pod-reference" class="pod-switch" data-template="#podref-tmpl" data-target="#pod-reference-wrapper" data-action="pq_loadpod" data-nonce="' . esc_attr( wp_create_nonce( 'pods_pq_loadpod' ) ) . '" data-event="change" />';
