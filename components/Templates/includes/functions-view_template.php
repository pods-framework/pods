...
function frontier_do_shortcode( $content ) {
	...
	$content = pods_do_shortcode( $content, array(
		'each',
		'pod_sub_template',
		'once',
		'pod_once_template',
 ...
 * @param array $atts attributed provided from parent
 *
 * @return string
 * @since 2.4
 */
...
function frontier_if_block( $atts, $code ) {
	...
	$template = pods_do_shortcode( $pod->do_magic_tags( $code[ 0 ] ), array(
		'each',
		'pod_sub_template',
		'once',
		'pod_once_template',
	...
	if ( ! empty( $field_data ) || isset( $atts[ 'value' ] ) ) {
		// There is a field - let go deeper
		if ( isset( $atts[ 'value' ] ) ) {
			...
			if ( $field_data == $atts[ 'value' ] ) {
				return pods_do_shortcode( $template, array(
					'each',
					'pod_sub_template',
					'once',
					'pod_once_template',
				...
				if ( isset( $code[ 1 ] ) ) {
					$template = pods_do_shortcode( $pod->do_magic_tags( $code[ 1 ] ), array(
						'each',
						'pod_sub_template',
						'once',
						'pod_once_template',
 ...
 * @param array $atts attributes from template
 * @param string $code encoded template to be decoded
 * @param string $slug shortcode slug used to process
 *
 * @return null
 * @since 2.4
 */
 ...
 *
 * @param array $atts attributes from template
 * @param string $content template to be processed
 *
 * @return null
 * @since 2.4
 */
 ...
 *
 *
 * @param array $code attributes from template
 * @param string $template template to be processed
 * @param $pod
 *
 * @return null
 *
...
function frontier_end_template( $code, $base, $template, $pod ) {
	...
	return pods_do_shortcode( $code, array(
		'each',
		'pod_sub_template',
		'once',
		'pod_once_template',
