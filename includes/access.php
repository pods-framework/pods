...
function pods_access_callback_allowed( $callback, array $params = [] ): bool {
	...
	if ( defined( 'PODS_DISPLAY_CALLBACKS' ) ) {
		...
		$display_callbacks = pods_get_setting( 'display_callbacks', 'restricted' );
	}
	...
	if ( 'customized' === $display_callbacks ) {
		if ( defined( 'PODS_DISPLAY_CALLBACKS_ALLOWED' ) ) {
			...
			$display_callbacks_allowed = pods_get_setting( 'display_callbacks_allowed', '' );
		}
		...
		$display_callbacks_allowed = array_values(
			array_filter(
				$display_callbacks_allowed,
				static function ( $name ) use ( $custom_prefix ) {
	...
	if ( is_string( $callback ) ) {
		$callback = wp_strip_all_tags( str_replace( [ '`', chr( 96 ) ], "'", $callback ) );
	}
 ...
 */
function pods_get_disallowed_display_callbacks(): array {
 ...
 */
function pods_access_record_disallowed_display_callback( string $callback ): void {
 ...
 * @since 3.3.9.2
 */
function pods_access_clear_disallowed_display_callbacks(): void {
...
function pods_access_settings_config(): array {
	...
	$fields['display_callbacks'] = [
		...
		'description'                 => $did_init ? __( 'Callbacks can be used when using Pods Templating syntax like {@my_field,my_callback} in your magic tags. You can allow additional callbacks through PHP by filtering the array using the filter "pods_helper_allowed_callbacks".', 'pods' ) : '',
		'type'                        => 'pick',
		'default'                     => 'restricted',
		'pick_format_type'            => 'single',
		'pick_format_single'          => 'radio',
		'data'                        => [
			'restricted' => $did_init ? __( 'Restricted - Only a built-in list of safe display and escaping functions are allowed.', 'pods' ) : '',
			'customized' => $did_init ? __( 'Customized - The restricted functions plus your own list of PHP function callbacks.', 'pods' ) : '',
	...
	$fields['display_callbacks_allowed'] = [
		...
		'description'                 => $did_init ? __( 'Please provide a comma-separated list of additional PHP function names to allow in callbacks, on top of the built-in safe list. Each additional function name must start with "custom_pods_callback_" and any other names are ignored for security purposes. You may choose to add custom PHP filter for "pods_access_callbacks_custom_prefix" to change this prefix.', 'pods' ) : '',
		'type'                        => 'text',
		'default'                     => '',
		'depends-on'                  => [
			'dynamic_features_allow' => '1',
	...
	$fields['show_display_callback_notices'] = [
		'name'                        => 'show_display_callback_notices',
		'label'                       => $did_init ? __( 'Display callback notices', 'pods' ) : '',
		'description'                 => $did_init ? __( 'When enabled, Pods will record disallowed display callbacks as they are detected and show an admin notice on the Pods Settings page listing those callbacks.', 'pods' ) : '',
		'type'                        => 'pick',
		'default'                     => '1',
		'pick_format_type'            => 'single',
		'pick_format_single'          => 'radio',
		'data'                        => [
			'1' => $did_init ? __( 'Enable admin notices when disallowed display callbacks are detected', 'pods' ) : '',
			'0' => $did_init ? __( 'Disable admin notices when disallowed display callbacks are detected', 'pods' ) : '',
		],
		'site_health_data'            => [
			'1' => $did_init ? __( 'Enable', 'pods' ) : '',
			'0' => $did_init ? __( 'Disable', 'pods' ) : '',
 ...
 */
function pods_access_sql_fragment_is_allowed( string $sql, string $context, array $args = [], ?object $params = null ): bool {
 ...
 * @return bool Whether the SQL fragment is allowed to be used.
 */
function pods_access_sql_fragment_disallow_post_status( bool $allowed, string $sql, string $context, array $info, ?object $params = null ): bool {
	if ( ! $allowed ) {
		return $allowed;
	}
	...
	if ( empty( $params ) || empty( $params->from ) || ! in_array( $params->from, [ 'dynamic-embed', 'pick/get_object_data' ], true ) ) {
		return true;
	}
