 ...
 * @return string
 */
function pods_shortcode_run_safely( array $tags, $content = null, $check_display_access_rights = true ) {
...
function pods_permission( $options ) {
	...
	if ( ! $permission && 1 === (int) pods_v( 'restrict_role', $options, 0 ) ) {
		$roles = pods_maybe_safely_unserialize( pods_v( 'roles_allowed', $options ) );
	...
	if ( ! $permission && 1 === (int) pods_v( 'restrict_capability', $options, 0 ) ) {
		$capabilities = pods_maybe_safely_unserialize( pods_v( 'capability_allowed', $options ) );
