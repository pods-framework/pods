...
<?php
    ...
    if(!in_array('delete',$object->ui['disable_actions'])&&$object->id!=null&&$duplicate==0)
        $form = str_replace('<input type="button" class="button btn_save" value="'.esc_attr($object->ui['label']).'" onclick="saveForm(1)" />','<input type="button" class="button btn_save" value="'. esc_attr($object->ui['label']).'" onclick="saveForm(1)" />&nbsp;&nbsp;&nbsp;<a style="color:#red;font-weight:normal;text-decoration:underline;font-style:italic;" title="Delete this item" href="'.pods_ui_var_update(array('action'.$object->ui['num']=>'delete','id'.$object->ui['num']=>$object->get_field('id'))).'" onclick="if ( confirm(\'You are about to delete this item \\\''. esc_attr(htmlentities($object->get_field('name'))).'\\\'\n \\\'Cancel\\\' to stop, \\\'OK\\\' to delete.\') ) { return true;}return false;">Delete this item</a>',$form);
    if($duplicate==1) {
