...
* Added: Multi-level references in field names when referenced in Pod :: findRecords $params (`select, where, groupby, having, orderby`) - ex. `'where' => 'pick_field.another_pick_field.another_pick.field = "Example"'`
* Added: Multi-level references in search filters when referenced in Pod :: findRecords $params (`select, where, groupby, having, orderby`) or Pod :: traverse variable (when not mentioned in params but you want it filterable)
...
* Added: PODS_GLOBAL_POD_SEARCH_MODE constant to globally set the search_mode to 'text' or 'int' (default 'int' which references field IDs) - can be overrided as needed in findRecords $params `'search_mode' => 'text'`)
