...
<div class="wrap pods-admin">
	...
	<form action="" method="post" class="pods-submittable pods-nav-tabbed">
		<div class="pods-submittable-fields">
			...
			<h2>
				<?php esc_html_e( 'Edit Pod', 'pods' ); ?>: <?php
			...
			?> <em><?php echo esc_html( $pod['name'] ); ?></em> <?php
			} else {
				...
				<div id="post-body-content" class="pods-nav-tab-group">
						...
						<table class="widefat fixed pages" cellspacing="0">
							...
							<tr>
								...
								<th scope="col" id="label" class="manage-column field-label">
										<span><?php esc_html_e( 'Label', 'pods' );
										pods_help( "<h6>" . __( 'Label', 'pods' ) . "</h6>" . __( 'The label is the descriptive name to identify the Pod field.', 'pods' ) ); ?></span>
								</th>
								<th scope="col" id="machine-name" class="manage-column field-machine-name">
										<span><?php esc_html_e( 'Name', 'pods' );
										pods_help( "<h6>" . __( 'Name', 'pods' ) . "</h6>" . __( 'The name attribute is what is used to identify and access the Pod field programatically.', 'pods' ) ); ?></span>
								</th>
								<th scope="col" id="field-type" class="manage-column field-field-type">
										<span><?php esc_html_e( 'Field Type', 'pods' );
										pods_help( "<h6>" . __( 'Field Type', 'pods' ) . "</h6>" . __( 'Field types are used to determine what kind of data will be stored in the Pod. They can range from, dates, text, files, etc.', 'pods' ) ); ?></span>
								</th>
							</tr>
							</thead>
							<tfoot>
					...
					<div id="pods-advanced" class="pods-nav-tab pods-manage-field pods-dependency pods-submittable-fields">
						...
						<div class="pods-field-option-group">
							...
							<div class="pods-pick-values pods-pick-checkbox">
								<ul>
									...
									<li>
										<div class="pods-field pods-boolean">
											<?php echo PodsForm::field( 'supports_thumbnail', pods_v( 'supports_thumbnail', $pod, false ), 'boolean', array(
												'boolean_yes_label' => __( 'Featured Image', 'pods' ),
												'dependency'        => true
											) ); ?>
										</div>
									</li>
						...
						<div class="pods-field-option">
							<?php echo PodsForm::label( 'supports_custom', __( 'Advanced Supports', 'pods' ), __( 'Comma-separated list of custom "supports" values to pass to register_post_type.', 'pods' ) ); ?> <?php echo PodsForm::field( 'supports_custom', pods_v( 'supports_custom', $pod, '' ), 'text' ); ?>
						</div>
						<div class="pods-field-option-group">
							<p class="pods-field-option-group-label">
						...
						?>
						<div class="pods-field-option">
							<?php echo PodsForm::label( 'detail_url', __( 'Detail Page URL', 'pods' ), __( 'help', 'pods' ) ); ?> <?php echo PodsForm::field( 'detail_url', pods_v( 'detail_url', $pod ), 'text' ); ?>
						</div>
						...
						<div class="pods-field-option">
							<?php echo PodsForm::label( 'pod_index', __( 'Title Field', 'pods' ), __( 'If you delete the "name" field, we need to specify the field to use as your primary title field. This field will serve as an index of your content. Most commonly this field represents the name of a person, place, thing, or a summary field.', 'pods' ) ); ?> <?php echo PodsForm::field( 'pod_index', pods_v( 'pod_index', $pod, 'name' ), 'pick', array( 'data' => $index_fields ) ); ?>
						</div>

						<div class="pods-field-option">
							<?php echo PodsForm::label( 'hierarchical', __( 'Hierarchical', 'pods' ), __( 'help', 'pods' ) ); ?> <?php echo PodsForm::field( 'hierarchical', (int) pods_v( 'hierarchical', $pod, 0 ), 'boolean', array(
							'dependency'        => true,
							'boolean_yes_label' => ''
						) ); ?>
						</div>
						...
						<div class="pods-field-option pods-depends-on pods-depends-on-hierarchical">
							<?php echo PodsForm::label( 'pod_parent', __( 'Hierarchical Field', 'pods' ), __( 'help', 'pods' ) ); ?> <?php echo PodsForm::field( 'pod_parent', pods_v( 'pod_parent', $pod, 'name' ), 'pick', array( 'data' => $hierarchical_fields ) ); ?>
						</div>
					...
					<div id="side-info-field" class="inner-sidebar">
						<div id="side-sortables">
							<div id="submitdiv" class="postbox pods-no-toggle">
								<h3><span><?php esc_html_e( 'Manage', 'pods' ); ?>
										<small>(<a href="<?php echo esc_url( pods_query_arg( array(
										...
										<div id="major-publishing-actions">
											<div id="delete-action">
												<a href="<?php echo esc_url( pods_query_arg( array(
													'action' . $obj->num => 'delete',
													'_wpnonce'           => wp_create_nonce( 'pods-ui-action-delete' )
												) ) ); ?>" class="submitdelete deletion pods-confirm" data-confirm="<?php _e( 'Are you sure you want to delete this Pod? All fields and data will be removed.', 'pods' ); ?>"> <?php _e( 'Delete', 'pods' ); ?> Pod </a>
											</div>
...
<script type="text/javascript">
	...
	var pods_sister_field = function ( $el ) {
		...
		if ( 0 != relatedPodName.indexOf( 'pod-' ) && 0 != relatedPodName.indexOf( 'post_type-' ) && 0 != relatedPodName.indexOf( 'taxonomy-' ) && 0 != relatedPodName.indexOf( 'user' ) && 0 != relatedPodName.indexOf( 'media' ) && 0 != relatedPodName.indexOf( 'comment' ) ) {
		...
		selectField = new PodsDFV.fields.Pick( {
												   el : $container, model : fieldModel, collection : collection
											   } );
