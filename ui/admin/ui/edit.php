...
<div class="wrap pods-ui">
	...
	<h2>
		...
		if ( ! in_array( 'add', $ui->actions_disabled ) && ! in_array( 'add', $ui->actions_hidden ) && ! $ui->restricted( 'add' ) ) {
			...
			?>
			<a href="<?php echo esc_url( $link ); ?>" class="add-new-h2"><?php echo wp_kses_post( $ui->heading['add'] ); ?></a>
			<?php
			...
			?>
			<a href="<?php echo esc_url( $link ); ?>" class="add-new-h2">&laquo; <?php echo sprintf( __( 'Back to %s', 'pods' ), $ui->heading['manage'] ); ?></a>
			<?php
		}//end if
		?>
	</h2>
