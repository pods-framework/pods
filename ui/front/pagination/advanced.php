...
if ( 1 < $params->page ) {
	...
	<?php if ( $params->first_last ) { ?>
			<a href="<?php echo esc_url( pods_query_arg( array( $params->page_var => 1 ) ) ); ?>" class="pods-pagination-first <?php echo esc_attr( $params->link_class ); ?>"><?php echo $params->first_text; ?></a>
		<?php } ?>
	<?php if ( $params->prev_next ) { ?>
			<a href="<?php echo esc_url( pods_query_arg( array( $params->page_var => ( $params->page - 1 ) ) ) ); ?>" class="pods-pagination-prev <?php echo esc_attr( $params->link_class ); ?>"><?php echo $params->prev_text; ?></a>
		<?php } ?>
...
for ( $i = $params->mid_size; $i > 0; $i -- ) {
	...
	if ( $params->page < $params->total ) {
		...
		if ( $params->prev_next ) {
			?>
			<a href="<?php echo esc_url( pods_query_arg( array( $params->page_var => ( $params->page + 1 ) ) ) ); ?>" class="pods-pagination-next <?php echo esc_attr( $params->link_class ); ?>"><?php echo $params->next_text; ?></a>
		<?php } ?>
		<?php if ( $params->first_last ) { ?>
			<a href="<?php echo esc_url( pods_query_arg( array( $params->page_var => $params->total ) ) ); ?>" class="pods-pagination-last <?php echo esc_attr( $params->link_class ); ?>"><?php echo $params->last_text; ?></a>
		<?php } ?>
		<?php
	}
	?>
