...
function pods_attachments ( src, file_limit ) {
    ...
    for ( var x = 0, len = wp_media_show_links.length; x < len; x++ ) {
        ...
        if ( wp_media_show.data( 'pods-injected-quick-add') !== true ) {
            // Create 'Add' link
            var pods_file_quick_add = jQuery( '<a href="#">' + podsLocalizedStrings.__add + '</a>' ).addClass( 'pods-quick-add' );
            ...
            pods_file_quick_add.bind( 'click', function( e ) {
                ...
                item.fadeOut( 'fast', function() {
                    ...
                    item.before( '<span class="pods-attached pods-quick-add">' + podsLocalizedStrings.__added + '</span>' );
    ...
    function pods_thickbox_send ( wp_media_meta, e ) {
        ...
        if ( 1 < file_limit || file_limit == 0 ) {
            jQuery( this ).after( ' <span class="pods-attached">' + podsLocalizedStrings.__added_choose_another_or_close_this_box + '</span>' );
