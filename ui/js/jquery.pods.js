		...
		methods = {
			...
			collapsible : function ( row ) {
				...
				$( 'tbody.pods-manage-list' ).on( 'click', 'a.pods-manage-row-edit', function ( e ) {
					...
					methods.collapsible_( $this.closest( 'tr.pods-manage-row' ) );
				...
				.on( 'click', '.pods-manage-row-save a.button-primary', function ( e ) {
					...
					methods.collapsible_save( $this.closest( 'tr.pods-manage-row' ) );
				...
				.on( 'click', '.pods-manage-row-actions a.pods-manage-row-cancel', function ( e ) {
					...
					methods.collapsible_cancel( $( this ).closest( 'tr.pods-manage-row' ) );

					e.preventDefault();

				} );
			},
			collapsible_show : function( $row ) {



			},
			collapsible_save : function( $row ) {
				...
				if ( valid_form ) {
					...
					if ( 'undefined' != typeof pods_field_types && null !== pods_field_types ) {
						...
						$row.find( 'td.pods-manage-row-type' ).html( field_type
																	 + field_type_desc
																	 + ' <span class="pods-manage-row-more">[type: '
																	 + $row_content.find( 'select#pods-form-ui-field-data-'
																						  + row_id + '-type' ).val()
																	 + ']</span>' );
					}
			...
			collapsible_cancel : function( $row ) {

				$row.find( 'a.pods-manage-row-edit' ).click();
